...
const unsigned char icc_exp[] = {
		0x03,
};

const unsigned char sdad_cr[] = {
...
static int cda_test_raw(void)
{
	...
	iccpk_data = crypto_pk_encrypt(ikcp, icc_cert, sizeof(icc_cert), &iccpk_data_len);
	crypto_pk_close(ikcp);

	if (!iccpk_data)
		return 1;

	dump_buffer(iccpk_data, iccpk_data_len, stdout);
	...
	crypto_hash_write(ch, icc_exp, sizeof(icc_exp));
	...
	size_t sdad_len;
	unsigned char *sdad = crypto_pk_encrypt(icckcp, sdad_cr, sizeof(sdad_cr), &sdad_len);
	crypto_pk_close(icckcp);
	if (!sdad)
		return 1;

	dump_buffer(sdad, sdad_len, stdout);
	...
	crypto_hash_write(ch, dd1, sizeof(dd1));
...
static int cda_test_pk(void)
{
	...
	dump_buffer(idn->value, idn->len, stdout);
